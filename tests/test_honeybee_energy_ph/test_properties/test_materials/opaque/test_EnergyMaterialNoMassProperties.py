from honeybee_energy_ph.properties.materials.opaque import EnergyMaterialNoMassPhProperties
from honeybee_ph_utils.color import PhColor


def test_energy_material_no_mass_ph_properties() -> None:
    ...
    d = new_prop.to_dict()
    new_prop = EnergyMaterialNoMassPhProperties.from_dict(d["ph"], None)
    assert new_prop.to_dict() == d
    assert new_prop.id_num == 0
    assert new_prop.ph_color is None


def test_energy_material_no_mass_ph_properties_with_color() -> None:
    new_prop = EnergyMaterialNoMassPhProperties(None)
    new_prop.id_num = 0
