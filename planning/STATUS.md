...
|------|------|--------|---------|
| `PhEquipment` defaulted `quantity` to 0 | Bug fix (data model; companion [PHX#153](https://github.com/PH-Tools/PHX/issues/153)) | **Complete** — [#129](https://github.com/PH-Tools/honeybee_ph/issues/129). Bare constructors now give `quantity = 1`; the three custom classes' `annual_energy_kWh()` returns `energy_demand * quantity` (`energy_demand` is per unit). `from_dict` unchanged, so legacy HBJSON keeps its stored 0; PHX#153 normalizes that on read. 1,105 tests pass at 81% coverage; PHX's 1,468 pass against the change | issue only |
| Retire `ElectricEquipmentPhProperties.equipment_collection` | Deprecation (cross-repo, **primary**) | **Complete** — closed [#79](https://github.com/PH-Tools/honeybee_ph/issues/79) 2026-09-12. Step 1 (load-time migration) released in v1.33.65 ([#124](https://github.com/PH-Tools/honeybee_ph/pull/124)); PHX dropped its reads in 1.56.108 ([PHX#124](https://github.com/PH-Tools/PHX/issues/124)); step 3 (attribute removed) released in v1.33.66 ([#126](https://github.com/PH-Tools/honeybee_ph/pull/126)). Rhino check passed: the legacy fixture loaded through HB Load Objects with all 12 devices on Process loads and no `equipment_collection` on export. GH component deletion ([honeybee_grasshopper_ph#84](https://github.com/PH-Tools/honeybee_grasshopper_ph/issues/84)) ships with the next GH release | [decision 0009](../context/decisions/0009-legacy-equipment-collection-migrates-on-load.md) |
| ISO 6946 mean-of-limits U-value for opaque constructions | Feature (filed from PH-Navigator SketchUp) | **Complete** — archived 2026-09-10. #116 as filed could not be built: a single layer's two ISO 6946 limits are the same number, so the calculation shipped on the construction instead. Merged in [#117](https://github.com/PH-Tools/honeybee_ph/pull/117) | [`archive/iso-6946-mean-of-limits/`](archive/iso-6946-mean-of-limits/STATUS.md) |
