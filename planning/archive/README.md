...
|------|------|-----------|---------|--------|
| Multiple ventilation systems per room | Refactor (investigated, declined) | 2026-08-27 | Investigated and decided NOT to implement; the one-system-per-room contract stays. Decision 0001. Closed as not-planned in [#113](https://github.com/PH-Tools/honeybee_ph/issues/113). | [`multiple-ventilation-systems.md`](multiple-ventilation-systems.md) |
| `phius_default()` shared mutable singleton | Bug fix (data model) | 2026-08-25 | The default factories cached one instance per class and handed that same object to every caller for the life of the process, with no `duplicate()` to escape it — a mutation in one Rhino model followed the user into the next. Fixed with an identifier-**preserving** `duplicate()`: the shared identity is what collapses the Phius-MF per-room devices to one device per PhxZone, so re-keying would have multiplied the exported energy by the room count. Decision 0008. | [`phius-default-shared-singleton/`](phius-default-shared-singleton/README.md) |
