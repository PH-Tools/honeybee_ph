...
**Do NOT include `index.md` in `nav.yml`.** The `index.md` file is the library landing page (card view) — it is accessed via the "LIBRARY · {name}" label at the top of the sidebar, not as a numbered nav item. Only list content sections in `nav.yml`.

The order of entries in `nav.yml` determines both sidebar numbering and card numbering — they always match. First entry = `01`, second = `02`, etc.

Top-level groups become numbered, collapsible sidebar sections. Top-level leaves become numbered sidebar links. Nested groups are supported.
...
Card numbers are derived automatically from position in `nav.yml` — first group = `01`, second = `02`, etc. This matches the sidebar numbering. Only the first file in each top-level group needs these fields.
...
- **Every `.md` file except `index.md` must be listed in `nav.yml`** — unlisted files won't be discovered or rendered. `index.md` is the landing page and is linked automatically via the sidebar library label
