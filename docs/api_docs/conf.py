...
def create_cli_files():
    ...
    if not update_cli_index(os.path.join(doc_folder, "index.rst"), list(ht_cli.keys())):
        return -1
...
def write_cli_files(ht_cli_data, lib_name, tool_name, doc_folder):
    ...
    for file in group_filenames:
        ...
        if file != "main":
            cli_content += [
                "\n",
                ".. click:: {}.cli.{}:{}\n".format(lib_name, file, ht_cli_data[file][0]),
                "   :prog: {} {}\n".format(tool_name, ht_cli_data[file][1]),
                "   :show-nested:\n",
            ]
        else:
...
def update_doc_index(proj_folder, lib_name):
    ...
    if m:
        ...
        print(
            "[CLI doc\\index]: index.rst update not possible - content \
